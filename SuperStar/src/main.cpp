...
void setup() {
  ...
  if (isBlueTeam()) {
    Serial.println("Blue team selected");
  } else {
    Serial.println("Yellow team selected");
  }
  ...
  delay(1000); // TODO: Wait for 86 second before starting
...
void superstar()
{
  if (isTargetReached()) {
    ...
    {
    case 0:
      setMotorTarget(1.3); // Move forward 1.3m
      next_point++;
      break;
    case 1:
      setTargetAngle(PI/2); // Turn
      next_point++;
      break;
    case 2:
      setMotorTarget(0.4); // Move forward
      next_point++;
      break;
    default:
      setMotorSpeeds(0, 0);
...
void loop() {
    ...
    if (millis() - lastUpdateTime >= controlInterval) {
      ...
      if (d<0.1)
      {
        ...
        Serial.println("Obstacle cleared, resuming motors.");
        // Resume motors after obstacle is cleared
        resumeMotors();
      }
      superstar();
