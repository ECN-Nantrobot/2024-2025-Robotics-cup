...
int main()
{
    ...
    std::vector<Point> goals = { Point(280, 180), Point(5, 5), Point(80, 150), Point(225, 80) }; // Example goal points
    ...
    while (true) {
        ...
        // // double left_wheel_speed  = robot.getLeftWheelSpeed();
        // // double right_wheel_speed = robot.getRightWheelSpeed();
        // double left_wheel_speed  = 0.2; // m/s (Anpassen!)
        // double right_wheel_speed = 0.1; // m/s (Anpassen!)
        // std::cout << "DEBUG: left_wheel_speed=" << left_wheel_speed << ", right_wheel_speed=" << right_wheel_speed << std::endl;
        // std::cout.flush();

        // fprintf(stdout, "DEBUG: left_wheel_speed=%.2f, right_wheel_speed=%.2f\n", left_wheel_speed, right_wheel_speed);
        // fflush(stdout);


        // RCLCPP_INFO(node->get_logger(), "Linke Radgeschwindigkeit: %.2f m/s, Rechte Radgeschwindigkeit: %.2f m/s", left_wheel_speed, right_wheel_speed);

        // geometry_msgs::msg::Twist msg;
        // msg.linear.x  = (left_wheel_speed + right_wheel_speed) / 2.0; // Durchschnittliche Geschwindigkeit
        // msg.angular.z = (right_wheel_speed - left_wheel_speed) / 0.3; // 0.3m Radabstand (Anpassen!)

        // msg.linear.x  = 0.1; // Durchschnittliche Geschwindigkeit
        // msg.angular.z = 0.4; // 0.3m Radabstand (Anpassen!)


        // // RCLCPP_INFO(node->get_logger(), "Linke Radgeschwindigkeit: %.2f m/s, Rechte Radgeschwindigkeit: %.2f m/s", left_wheel_speed, right_wheel_speed);

        // RCLCPP_INFO(node->get_logger(), "Sende /cmd_vel: Linear=%.2f m/s, Angular=%.2f rad/s", msg.linear.x, msg.angular.z);
